...
#define HID_DEMO_TAG "Anto's Keyboard"
...
#define HIDD_DEVICE_NAME            "Anto's Keyboard"
...
static void hidd_event_callback(esp_hidd_cb_event_t event, esp_hidd_cb_param_t *param)
{
    switch(event) {
		...
		case ESP_HIDD_EVENT_BLE_CONNECT: {
            ...
            keyboard_set_connection(hid_conn_id);
            break;
        }
...
void hid_demo_task(void *pvParameters)
{
    while (1)
    {
        /* Wait until a BLE connection is established */
        if (!sec_conn)
        {
        ...
        /* USB keyboard will send keys instead */
